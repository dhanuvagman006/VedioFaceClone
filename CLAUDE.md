...
| `lipsync.py` | MuseTalk 1.5: face video → cached avatar in `avatars\<name>-<hash>\` (frames, FAN-landmark boxes, 5-point face landmarks, `found.npy`, VAE latents, blend masks) → `render()` streams lip-synced frames into ffmpeg; with `restore` > 0 a second pass sharpens the mouth. Also `facing_camera()`, used by both engines: the longest stretch where the face is found and not turned away (nose-tip offset from the eyes' centre line, in eye distances, vs. the usual pose; `TURN`, `SIDE`), so a moment of looking away never shows up. Caches without `found.npy` count every frame as found |
...
| `latentsync_faces.py` | runs inside LatentSync's env like the runner: per frame, whether LatentSync's own `FaceDetector` would accept a face (same InsightFace model, size, RGB input and limits; score ≥ 0.6 vs its 0.5) and its 5 points |
| `latentsync.py` | LatentSync 1.6 orchestration (our env): `prepare_face()` checks every frame with `latentsync_faces.py`, cuts to `facing_camera()`'s stretch and caches it as 25 fps 720p forward+backward (`avatars\...\latentsync_cycle.mp4`, `info.json` with `used_frames`), `render()` loops it to the speech, splits into 30 s parts of exact frame counts, runs the runner per part, joins, muxes the full-quality voice + AI tag. `usable()` checks GPU memory with nvidia-smi |
...
5. Colab can't be run from here. Keep `setup.sh`/`speak.sh` LF-only (`.gitattributes`) and check them with `bash -n`.
   `facing_camera` is plain NumPy: feed it 5-point faces projected from a 3D head with scripted yaw (a turn, lost frames, an off-axis camera, a tilted head, a photo) and check the chosen stretch.
   LatentSync's orchestration runs on CPU: replace `latentsync._run` with ffmpeg keeping the first `frames` frames of its input (the runner's contract) and `latentsync._find_faces` with synthetic faces, point `AVATARS_DIR`, `model_dir` and `installed` at scratch, and check with synthetic videos whose frames each have a distinct brightness that every frame lands in place across loops and 30 s parts (use part lengths whose cuts avoid the forward/backward turnaround, where frames repeat). Its GPU side only runs on Colab.

...
| `lipsync.py` | MuseTalk 1.5: face video → cached avatar in `avatars\<name>-<hash>\` (frames, FAN-landmark boxes, VAE latents, blend masks) → `render()` streams lip-synced frames into ffmpeg |
...
| `models.py`, `download.py` | model repos (`LIPSYNC_FILES`), `COMMAND` name per OS, one-time downloads into `models\` |
...
- **`qwen_fast.py` was verified to produce identical logits (float32) to the original code.** Re-verify the same way after any change there. Likewise `lipsync._blend` is pixel-identical to MuseTalk's `get_image_blending`, and tracked face boxes were within ~1 px of per-frame detection.
...
- Licences: F5-TTS weights are CC-BY-NC (non-commercial); Qwen3-TTS is Apache-2.0; MuseTalk code and weights are MIT/commercial-OK. Outputs carry an `AI-generated` metadata tag; keep it.
...
- `models\` (~12 GB of downloaded weights, incl. the generated `unet_fp16.safetensors` cache) and `outputs\` (generated audio/video): don't delete without asking.
